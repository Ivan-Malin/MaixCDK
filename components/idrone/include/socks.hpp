...
inline cv::Mat maix_image_to_cv_mat(maix::image::Image* img)
{
    ...
    else if (img->format() == maix::image::FMT_GRAYSCALE)
    {
        return cv::Mat(img->height(), img->width(), CV_8UC1, (void *)img->data());
    }
    else if (img->format() == maix::image::FMT_BGR565)
    {
        // Нужно сначала преобразовать RGB565 -> RGB888
        maix::image::Image* rgb888_img = img->to_format(maix::image::FMT_BGR888);
